...
public final class HiveSqlFormatter
{
    private static final String INDENT = "   ";

    private HiveSqlFormatter() {}
            ...
            extends AstVisitor<Void, Integer>
    {
        ...
        {
            if (node.getProperties().contains(EXTERNAL_LOCATION)) {
            ...
            builder.append('`');
            QualifiedName qTableName = node.getName();
            List<String> parts = qTableName.getParts();
            String tableName;
            if (parts.size() <= 2) {
                tableName = formatName(node.getName());
            } else {
                tableName = parts.get(1) + "." + parts.get(2);
            }
            ...
            Map<String, Expression> propertiesMap = new HashMap<>();
            node.getProperties().stream().forEach(e -> {
                propertiesMap.put(e.getName().toString(), e.getValue());
            });
            List<String> partitionKeys = getPropertyColumns(propertiesMap, PARTITIONED_BY);
            String columnList = node.getElements().stream().filter(element -> {
                if (element instanceof ColumnDefinition) {
                    ColumnDefinition column = (ColumnDefinition) element;
                    if (propertiesMap.containsKey(PARTITIONED_BY)) {
                        String columnNmme = column.getName().getValue();
                        if (partitionKeys.contains(columnNmme)) {
                            return false;
                        }
                    }
                    return true;
            ...
            builder.append(formatPropertiesMultiLine(propertiesMap, partitionKeys));

            return null;
        }

        List<String> getPropertyColumns(Map<String, Expression> propertiesMap, String key) {
            ArrayConstructor arrayConstructor = (ArrayConstructor)propertiesMap.get(key);
            if (arrayConstructor == null || arrayConstructor.getValues() == null) {
                return new ArrayList<>();
            }
            List<String> columns = new ArrayList<>();
            arrayConstructor.getValues().stream().forEach(e -> {
                StringLiteral stringLiteral = (StringLiteral)e;
                columns.add(stringLiteral.getValue());
            });
            return columns;
        }
        ...
        private String formatPropertiesMultiLine(Map<String, Expression> properties, List<String> partitionKeys)
        {
            ...
            String format = String.valueOf(properties.get("format"));
            if (format != null && format.length() > 0) {
                format = format.replaceAll("'", "");
                stringBuilder.append(getFileFormat(format));
            }

            Expression location = properties.get(EXTERNAL_LOCATION);
            if (location != null) {
            ...
            if (bloomColumns.size() > 0) {
                ...
                tableProperties.append(String.join(",", bloomColumns));
                tableProperties.append("',\n");
                tableProperties.append(INDENT);
                tableProperties.append("'orc.bloom.filter.fpp'='");
                tableProperties.append(properties.get("orc_bloom_filter_fpp"));
                tableProperties.append("'\n");
            }

            if (tableProperties.length() > 0) {
                stringBuilder.append("\nTBLPROPERTIES (\n");
                stringBuilder.append(tableProperties);
                stringBuilder.append(")\n");
            }
            return stringBuilder.toString();
        }

        String getFileFormat(String format) {
            ...
            if (format == null || !immutableSet.contains(format)) {
                return "";
            }
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("\nROW FORMAT SERDE\n");
            switch (format) {
                case "ORC":
                    ...
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.orc.OrcSerde'");
                    break;
                case "PARQUET":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.parquet.serde.ParquetHiveSerDe'");
                    break;
                case "AVRO":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.serde2.avro.AvroSerDe'");
                    break;
                case "RCBINARY":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.serde2.columnar.LazyBinaryColumnarSerDe'");
                    break;
                case "RCTEXT":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.serde2.columnar.ColumnarSerDe'");
                    break;
                case "SEQUENCEFILE":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.serde2.lazy.LazySimpleSerDe'");
                    break;
                case "JSON":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hive.hcatalog.data.JsonSerDe'");
                    break;
                case "TEXTFILE":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.serde2.lazy.LazySimpleSerDe'");
                ...
                case "CSV":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.serde2.OpenCSVSerde'");
                    break;
            }
            ...
            switch (format) {
                case "ORC":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.orc.OrcInputFormat'");
                    break;
                case "PARQUET":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat'");
                    break;
                case "AVRO":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.avro.AvroContainerInputFormat'");
                    break;
                case "RCBINARY":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.RCFileInputFormat'");
                    break;
                case "RCTEXT":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.RCFileInputFormat'");
                ...
                case "SEQUENCEFILE":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.mapred.SequenceFileInputFormat'");
                    break;
                case "JSON":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.mapred.TextInputFormat'");
                    break;
                case "TEXTFILE":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.mapred.TextInputFormat'");
            ...
            switch (format) {
                case "ORC":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.orc.OrcOutputFormat'");
                    break;
                case "PARQUET":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'");
                    break;
                case "AVRO":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.avro.AvroContainerOutputFormat'");
                    break;
                case "RCBINARY":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.RCFileOutputFormat'");
                    break;
                case "RCTEXT":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.RCFileOutputFormat'");
                ...
                case "SEQUENCEFILE":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.HiveSequenceFileOutputFormat'");
                    break;
                case "JSON":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.HiveIgnoreKeyTextOutputFormat'");
                    break;
                case "TEXTFILE":
                    stringBuilder.append(INDENT);
                    stringBuilder.append("'org.apache.hadoop.hive.ql.io.HiveIgnoreKeyTextOutputFormat'");
        ...
        private String formatColumnDefinition(ColumnDefinition column)
        {
            StringBuilder sb = new StringBuilder()
                    ...
                    .append('`')
                    .append(" ").append(getColumnType(column.getType()));
        ...
        private String getColumnType(DataType type) {
            ...
            if (type instanceof RowDataType) {
                ...
                RowDataType.Field field = rowDataType.getFields().get(0);
                Identifier identifier = field.getName().get();
                String name = identifier.getValue().replaceAll("\"", "`");
                builder
                    .append("struct<");
                if (!name.startsWith("`")) {
                    builder.append('`');
                ...
                for (int i = 1; i < rowDataType.getFields().size(); ++i) {
                    field = rowDataType.getFields().get(i);
                    identifier = field.getName().get();
                    name = identifier.getValue().replaceAll("\"", "`");
                    builder
                            .append(",");
                    if (!name.startsWith("`")) {
                        builder.append('`');
                ...
                DateTimeDataType dateTimeDataType = (DateTimeDataType) type;
                builder.append(dateTimeDataType.getType());
            } else if (type instanceof GenericDataType) {
                GenericDataType genericDataType = (GenericDataType)type;
                if (genericDataType.getName().toString().equals("array")) {
                    builder.append("array<");
                    DataTypeParameter dataTypeParameter = genericDataType.getArguments().get(0);
                    if (dataTypeParameter instanceof TypeParameter) {
                        TypeParameter typeParameter = (TypeParameter)dataTypeParameter;
                        builder.append(getColumnType(typeParameter.getValue()));
                    } else if (dataTypeParameter instanceof NumericParameter) {
                        builder.append(dataTypeParameter);
                    }
                    builder.append('>');
                } else {
                    builder.append(type);
                ...
                IntervalDayTimeDataType intervalDayTimeDataType = (IntervalDayTimeDataType)type;
                builder.append(intervalDayTimeDataType.getFrom());
            } else {
                builder.append(type);
            }
            return builder.toString();
